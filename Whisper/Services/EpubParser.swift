...
class EpubParser: NSObject, XMLParserDelegate {
    ...
    func parse(sourceURL: URL) -> Book? {
        ...
        do {
            ...
            print("EpubParser: Manifest items: \(manifestItems.count), Spine items: \(spineItemRefs.count)")
            ...
            let prefix = relativeOpfPath.hasPrefix("/") ? String(relativeOpfPath.dropFirst()) : relativeOpfPath
                ...
                if fileManager.fileExists(atPath: tocURL.path) {
                    ...
                    let finalTOC = tocItems.map { item -> TOCItem in
                        ...
                        if !cleanTocPrefix.isEmpty && !pathWithoutFragment.hasPrefix("/") {
                            newPath = cleanTocPrefix.isEmpty ? pathWithoutFragment : "\(cleanTocPrefix)/\(pathWithoutFragment)"
                        }
            ...
            let book = Book(
                id: bookID,
                title: foundTitle.isEmpty ? sourceURL.deletingPathExtension().lastPathComponent : foundTitle,
